...
function (_EventEmitter) {
  ...
  function LiveClient(opts) {
    ...
    if (typeof opts === 'string') {
      ...
      if (opts.debug) {
        _this._debugLogs = true;
      }

      if (opts.cookiesFallback) {
        _this._cookiesFallback = true;
      }
    }
  ...
  _createClass(LiveClient, [{
     ...
     **/
    value: function start(threadId, sessionId, isReconnect, isNewClient) {
      try {
    ...
    value: function send(message) {
      ...
      this.log("Updating threadId in send, this.threadId=".concat(this.threadId, ", message.threadId=").concat(message.threadId));
      ...
      try {
        ...
        if (message.attachment && message.attachment instanceof _fileAttachment.default) {
          ...
          this._rest.upload(formData, headers).then(function (result) {
            if (result.status !== 'ok') {
        ...
        setTimeout(function () {
          ...
          // event get lost
          if (_this2._socket) _this2._socket.send(enveloppe);
        }, 50);
      } catch (err) {
    ...
    value: function sendUserTypingActivity(params) {
      ...
      try {
        ...
        setTimeout(function () {
          ...
          // messages instantly send after 'connection'
          // event get lost
          if (_this3._socket) _this3._socket.send(enveloppe);
        }, 50);
      } catch (err) {
        this.emit(LiveClient.ERROR, new _exception.default(err));
      }
    ...
    value: function history(threadId) {
      ...
      this.log("Updating threadId in history, this.threadId=".concat(this.threadId, ", threadId=").concat(threadId));
      this.threadId = threadId;
    ...
    value: function noticed(threadId, instantly) {
      ...
      this._noticedTimeout = setTimeout(function () {
        ...
        try {
          if (_this6._socket) _this6._socket.send(enveloppe);
        } catch (err) {
    ...
    value: function checkUnnoticed(threadId) {
      ...
      this.log("Updating threadId in checkUnnoticed, this.threadId=".concat(this.threadId, ", threadId=").concat(threadId));
      this.threadId = threadId;
    ...
    value: function log(text) {
      ...
      setTimeout(function () {
        ...
        // messages instantly send after 'connection'
        // event get lost
        if (_this8._socket) _this8._socket.send(enveloppe);
      }, 50);
    }
    /**
     * Setup the client
    ...
    key: "_reconnect",
    value: function _reconnect(reconnectSource) {
      var _this9 = this;
      ...
      this._reconnectTimeout = setTimeout(function () {
        _this9.emit(LiveClient.RECONNECTING);

        _this9._openConnection(true, reconnectSource);
      }, timeout);
    }
    /**
    ...
    value: function _openConnection() {
      ...
      var reconnectSource = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
      this._isAutoReconnect = true;
      var cookieThread = (0, _cookies.getCookie)("".concat(this._clientId, ".threadId")),
          cookieSession = (0, _cookies.getCookie)("".concat(this._clientId, ".sessionId")); // Request endpoint to give a WS url
      ...
      this._rest.get({
        ...
        if (typeof result.secret === 'string') {
          _this10.secret = result.secret;
        }

        _this10._handleConnection(result.payload);
      }).catch(function (err) {
        if (_this10._silent !== true) {
          console.error('LiveClient: Error while trying to connect', err);
        }

        if (!err.isFinal) {
          _this10._reconnect('rest');
        }

        _this10.emit(LiveClient.ERROR, new _exception.default(err, 'connection'));
      });
    }
    /**
    ...
    value: function _handleConnection(payload) {
      ...
      socket.onopen = function () {
        ...
        _this11.emit(LiveClient.CONNECTED); // Clear any running interval


        clearInterval(_this11._keepAliveInterval); // Create a new Interval

        _this11._keepAliveInterval = _this11._keepAlive();

        _this11.log('Connection opened');
      };
      ...
      socket.onerror = function (evt) {
        ...
        switch (socket.readyState) {
          case 0:
            {
              _this11.emit(LiveClient.ERROR, new _exception.default("".concat(msg, " Socket is busy connecting."), 'connection'));
          ...
          case 1:
            {
              _this11.emit(LiveClient.ERROR, new _exception.default("".concat(msg, " Socket is connected."), 'connection'));
          ...
          case 2:
            {
              _this11.emit(LiveClient.ERROR, new _exception.default("".concat(msg, " Connection is busy closing."), 'connection'));
          ...
          case 3:
            {
              _this11.emit(LiveClient.ERROR, new _exception.default("".concat(msg, " Connection is closed."), 'connection'));
          ...
          default:
            {
              _this11.emit(LiveClient.ERROR, new _exception.default(msg, 'connection'));
      ...
      socket.onclose = function (evt) {
        ...
        _this11.log('Connection closed');

        _this11._socket = null;

        if (evt && evt.code === 1006) {
          _this11.emit(LiveClient.ERROR, new _exception.default('The connection closed abnormally', 'connection', null, true));

          _this11.emit(LiveClient.DISCONNECTED); // DIRTY FIX? Need to check this in the future


          _this11._reconnect('abnormal');
        } else if (evt && evt.reason !== 'connection failed') {
          _this11.emit(LiveClient.DISCONNECTED);

          _this11._reconnect('connection failed');
        } else {
          _this11.emit(LiveClient.DISCONNECTED);
        }
      };
      ...
      socket.onmessage = function (evt) {
        ...
        if (typeof evt.data !== 'string' || evt.data.length == 0) {
          _this11.emit(LiveClient.ERROR, new _exception.default('Failed to receive a valid websocet URL, please check the clientId', 'connection'));
        ...
        switch (type) {
          ...
          case 'agent.typing':
            _this11._handleReceivedAgentTyping(payload);

            break;

          case 'message.received':
          case 'activities.created':
            _this11._handleReceived(payload);

            break;

          case 'error':
            _this11.emit(LiveClient.ERROR, new _exception.default(message, 'connection'));

            break;

          case 'message.delivered':
          case 'activities.delivered':
            _this11._handleDelivered(payload);
          ...
          case 'interruption.occurred':
            {
              _this11.log('Interruption occured');

              _this11._handleInterruptionOccurred();
          ...
          default:
            {
              ...
              _this11.emit(LiveClient.ERROR, new _exception.default("Unknown message received ".concat(evt.data), 'connection'));
            }
        }
      };
    ...
    key: "_handleInterruptionOccurred",
    value: function _handleInterruptionOccurred() {
      ...
      return setInterval(function () {
        try {
          if (_this12.isConnected) {
            ...
            _this12._socket.send(JSON.stringify({
              type: 'ping'
            }));
          }
        } catch (err) {
          if (_this12._silent !== true) {
    ...
    set: function set(value) {
      ...
      this.log("Setting sessionId=".concat(value));
    ...
    ,
    set: function set(value) {
      (0, _debug.default)("Creating a new threadId with value '".concat(value, "'"));
      this.log("Setting threadId=".concat(value)); // Create a new Thread
      ...
      this.log("ThreadId=".concat(this.threadId, " after setting value=").concat(value));
    }
    /**
     * Secret
     * @returns {?string}
    ...
    set: function set(value) {
      ...
      this._secret = new _unique.default({
        clientId: this._clientId,
        key: 'secret',
        value: value,
        engine: this._storage,
