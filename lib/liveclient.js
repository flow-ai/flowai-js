  ...
  _createClass(LiveClient, [{
    ...
    value: function send(message) {
      ...
      try {
        ...
        if (message.attachment && message.attachment instanceof _attachment.FileAttachment) {
          ...
          this._rest.upload(formData).then(function (result) {
            if (result.status !== 'ok') {
              ...
              _this2.emit(LiveClient.MESSAGE_DELIVERED, message);
            }
          }).catch(function (err) {
    ...
    value: function _handleConnection(payload) {
      ...
      try {
        socket = new _websocket.w3cwebsocket(endpoint, null, this._origin);
      } catch (err) {
        // When you fail to connect, die!
        throw new _exception2.default('Error failed to connect to endpoint', 'connection', null, true);
      }
      ...
      socket.onerror = function (evt) {
        (0, _debug2.default)('Socket onerror', evt);
        _this9.emit(LiveClient.ERROR, new _exception2.default('Error during connection', 'connection'));
      };
