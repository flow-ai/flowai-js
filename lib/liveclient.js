...
var LiveClient = function (_EventEmitter) {
   ...
   * @returns {LiveClient}
   */
  function LiveClient(clientId, endpoint) {
    ...
    _this._endpoint = endpoint || 'https://api.flow.ai/channels/webclient/api';
  ...
  _createClass(LiveClient, [{
    ...
    value: function start(threadId, sessionId) {
      try {
        ...
        // Wrap the error
        throw new _exception2.default("Failed to start the client", 'connection', err);
      }
    }
    ...
    key: 'merger',
    value: function merger(mergerKey, threadId) {
      var _this3 = this;
    ...
    value: function history(threadId) {
      ...
      if (!this.isConnected) {
        throw new _exception2.default("Could not load any historic information", 'user');
      }
    ...
    value: function noticed(threadId, instantly) {
      ...
      this._noticedTimeout = setTimeout(function () {
        ...
        var enveloppe = JSON.stringify({
          type: 'thread.noticed',
          payload: {
            threadId: threadId || _this5.threadId
          }
        });
    ...
    value: function checkUnnoticed(threadId) {
      ...
      this._rest.get({
        ...
        if (result.payload.unnoticed) {
          _this6.emit(LiveClient.UNNOTICED_MESSAGES);
        }
      }).catch(function (err) {
    ...
    value: function _openConnection() {
      ...
      this._rest.get({
        ...
        return _this8._handleConnection(payload);
      }).catch(function (err) {
    ...
    value: function _handleDelivered(payload) {
      ...
      this.emit(LiveClient.MESSAGE_DELIVERED, new _message2.default(payload));
    }
  }, {
    ...
    set: function set(value) {
      this._session = new _unique2.default('sessionId', value);
    }
    ...
    set: function set(value) {
      // Create a new Thread
      this._thread = new _unique2.default('threadId', value);
    }
