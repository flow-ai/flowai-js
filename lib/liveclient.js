  ...
  */
  function LiveClient(opts) {
    ...
    if (typeof _this._clientId !== 'string') {
      throw new _exception2.default("Invalid or lacking argument for LiveClient. You must provide a clientId. Check the dashboard", 'user');
    }
  ...
  _createClass(LiveClient, [{
    ...
    value: function start(threadId, sessionId) {
      try {
        ...
        this._backoff = new _backo2.default({
          min: 100,
          max: 20000
        });
    ...
    value: function send(message) {
      ...
      try {
        ...
        if (message.attachment && message.attachment instanceof _fileAttachment2.default) {
          ...
          (0, _debug2.default)('Uploading formData', formData);

          this._rest.upload(formData).then(function (result) {
            if (result.status !== 'ok') {
              _this2.emit(LiveClient.ERROR, new _exception2.default(new Error('Failed to upload file.'), 'connection'));
            } else {
              _this2.emit(LiveClient.MESSAGE_DELIVERED, result.payload);
            }
          }).catch(function (err) {
            (0, _debug2.default)('Error while trying to upload a file', err);
            _this2.emit(LiveClient.ERROR, new _exception2.default(err, 'connection'));
          });
    ...
    value: function _openConnection() {
      ...
      this._rest.get({
        ...
        if (result.status !== 'ok') {
          throw new _exception2.default('Unable to get a socket URL": ' + result.payload.message, 'connection');
        }
    ...
    value: function _handleConnection(payload) {
      ...
      socket.onerror = function (evt) {
        ...
        switch (socket.readyState) {
          case 0:
            {
              _this9.emit(LiveClient.ERROR, new _exception2.default(msg + ' Socket is busy connecting.', 'connection'));
              break;
            }
          case 1:
            {
              _this9.emit(LiveClient.ERROR, new _exception2.default(msg + ' Socket is connected.', 'connection'));
              break;
            }
          case 2:
            {
              _this9.emit(LiveClient.ERROR, new _exception2.default(msg + ' Connection is busy closing.', 'connection'));
              break;
            }
          case 3:
            {
              _this9.emit(LiveClient.ERROR, new _exception2.default(msg + ' Connection is closed.', 'connection'));
              break;
            }
          default:
            {
              _this9.emit(LiveClient.ERROR, new _exception2.default(msg, 'connection'));
              break;
            }
        }
      };
      ...
      socket.onclose = function (evt) {
        (0, _debug2.default)('Socket onclose', evt);

        _this9._socket = null;

        if (evt && evt.code === 1006) {
          _this9.emit(LiveClient.ERROR, new _exception2.default('It seems your domain is not whitelisted', 'connection', null, true));
    ...
    set: function set(value) {
      (0, _debug2.default)('Creating a new sessionId with value \'' + value + '\'');
    ...
    ,
    set: function set(value) {
      (0, _debug2.default)('Creating a new threadId with value \'' + value + '\'');
      // Create a new Thread
