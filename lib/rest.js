      ...
      case 400:
        {
          ...
          error = new _exception2.default('Failed to connect with API. Invalid clientId, sessionId or configuration', 'connection', new Error(response.status), true);
          break;
        }
      case 401:
      case 402:
      case 403:
        {
          ...
          error = new _exception2.default('Failed to connect with API. Your domain is not whitelisted', 'connection', new Error(response.status), true);
          break;
        }
      default:
        {
          console.error('An error occurred at the Flow.ai API. Please contact us at slack.flow.ai');
          error = new _exception2.default('Failed to connect with API', 'connection', new Error(response.status));
          break;
        }
    }
...
var Rest = function () {
  ...
  _createClass(Rest, [{
    ...
    value: function upload(formData) {
      ...
      (0, _debug2.default)('Uploading', formData);

      var url = this._endpoint + '/thread.upload';
    ...
    value: function _call(url, enveloppe, queryParams) {
      return new Promise(function (resolve, reject) {
        ...
        (0, _fetchEverywhere2.default)(url, enveloppe).then(checkStatus).then(function (response) {
          resolve(response.json());
        }).catch(function (err) {
          (0, _debug2.default)('Failed with error', err);
          reject(err);
